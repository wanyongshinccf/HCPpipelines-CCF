...
verbose_echo "  "
verbose_red_echo " ===> Running PostStepResampling_SLOMOCO"
verbose_echo " "
...
echo " "
echo " START: PostStepResampling_SLOMOCO"
...
then
    ...
    for ((k=0; k < $NumFrames; k++)); do
      ...
      # ${FSLDIR}/bin/convertwarp --relout --rel --ref=${WD}/prevols/vol${vnum}.nii.gz --warp1=${GradientDistortionField} --postmat=${MotionMatrixFolder}/${MotionMatrixPrefix}${vnum} --out=${MotionMatrixFolder}/${MotionMatrixPrefix}${vnum}_gdc_warp.nii.gz (CCF, W.S)
      # Add in the warp to MNI152
      #${FSLDIR}/bin/convertwarp --relout --rel --ref=${WD}/${T1wImageFile}.${FinalfMRIResolution} --warp1=${MotionMatrixFolder}/${MotionMatrixPrefix}${vnum}_gdc_warp.nii.gz --warp2=${OutputTransform} --out=${MotionMatrixFolder}/${MotionMatrixPrefix}${vnum}_all_warp.nii.gz (CCF, W.S)
      # Add in the wanp to MNI152 
      ${FSLDIR}/bin/convertwarp --relout --rel --ref=${WD}/${T1wImageFile}.${FinalfMRIResolution} \
      --warp1=${OutputTransform} \
      --out=${MotionMatrixFolder}/${MotionMatrixPrefix}${vnum}_all_warp.nii.gz
      ...
      #Do Basic Cleanup
      # rm ${MotionMatrixFolder}/${MotionMatrixPrefix}${vnum}_gdc_warp.nii.gz (CCF W.S)
